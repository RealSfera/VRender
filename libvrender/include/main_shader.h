...
const char *main_shader_source = STRINGIFY(
										   \n!!vs \n \
										   \n \
										   #version 130 \n \
										   \n \
										   ...
										   void main(void) { \n \
											   ...
											   \n \
											   // specular
											   vec4 specular = vec4( ((material_shininess+8)/8*3.1415926535) * pow(max(0.0, dot(h, n)), material_shininess) ) * coef_specular; \n \
											   \n \
											   //// Коэффициент для hemisphere lighting
											   float a = dot(n, l) * 0.5 + 0.5; \n \
											   \n \
											   //color += vec4(material_color, 1.0) * ambient * vec4(light_color, 1.0);
											   color += vec4(material_color, 1.0) * ambient * vec4(mix(vec3(0.2, 0.2, 0.2), vec3(1.0, 1.0, 1.0), a), 1.0); \n \
											   color += vec4(material_color, 1.0) * diffuse * vec4(light_color, 1.0); \n \
											   color += vec4(material_color, 1.0) * specular * vec4(light_spec_color, 1.0); \n \
											   \n \
											   ////////////// Коррекция гаммы
											   \n \
