...
static const QString build_help_text = QString::fromUtf8("<h2><center>Основы языка построения</center></h2>" \
								 "<p>	Данный язык был специально разработан для упрощения его изучения и простоты работы с ним. За основу были взяты выражения из языка программирования C, а также одна конструкция из Фортрана и несколько специализированных (которые не встречаются в других языках и выражениях).</p>" \
												   ...
												   "<p>Так как данный язык создавался в первую очередь для построения скалярных полей, то в нём предусмотрены специальные переменные, которым перед построением присваиваются значения. Переменные x, y, z являются встроенными и их значение равно координатам вычисляемой точки внутри скалярного поля. Их значения изменяются от 0 до (размер скалярного поля - 1). Эти переменные можно изменять, но это ни на что, не повлияет.</p>" \
...
static const QString program_help_text = QString::fromUtf8("<h2><center>Справка по программе VRender "VERSION"</center></h2>" \
												   "<p>Вот некоторые математические понятия: </p>" \
												   "<p>Скалярное поле: если каждой точке M некоторой области некоторого пространства поставлено в соответствие некоторое (обычно — действительное) число u, то говорят, что в этой области задано скалярное поле. Другими словами, скалярное поле — это функция, отображающая R^n в R (скалярная функция точки пространства). [Взято с Wikipedia].</p>" \
												   "<p>Изо-уровень - некоторое постоянное значение, которое находится в скалярном поле, и которое образует поверхность (если скалярное поле - трёхмерное). Таким образом, в каждой точке поверхности находится одно и то же значение (изо-уровень).</p>" \
												   "<p></p>" \
												   "<p>VRender - построитель и визуализатор скалярного поля. Построение скалярного поля производится вводом требуемой функции в поле Функция построения, выбором размера скалярного поля (по-умолчанию, установлен 128) и нажатии кнопки НАЧАТЬ ПОСТРОЕНИЕ. Визуализация скалярного поля производится автоматически в режиме реального времени, любые изменения настроек визуализации автоматически применяются. На вкладке Базовые можно настроить: Размер поля полигонизации, влияющее на детализацию объекта, Изо-уровень задаёт значение, из скалярного поля, на основе которого пороизводится полигонизация поверхности объекта, также можно установить Постпенное изменение - это пошаговое изменение изо-уровня, где можно задать значение от, до и шаг изменения.</p>" \
												   "<p></p>" \
												   "<p>На вкладке Освещение и цвет можно устанавливать цвет внутренней и внешней поверхности объекта, цвет источника света и цвет блика, причём сделать можно либо вручную устанавливая состовляющие цвета (от 0 до 255 для каждой состовляющей)</p>" \
												   "<p></p>" \
												   "<p></p>" \
												   "<p></p>");
